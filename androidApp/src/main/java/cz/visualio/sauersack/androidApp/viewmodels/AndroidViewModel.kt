package cz.visualio.sauersack.androidApp.viewmodels
...
import cz.visualio.sauersack.androidApp.shared.viewmodels.MVI
import cz.visualio.sauersack.androidApp.shared.viewmodels.MainMVI
import java.util.*
...
fun getPreferredLocaleList(): List<Locale> {
    ...
    return (0 until adjustedLocaleListCompat.size()).fold(mutableListOf(), { acc, index ->
        adjustedLocaleListCompat.get(index)?.let { acc.add(it) }
        acc
    })
}
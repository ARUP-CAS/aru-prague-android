...
import cz.visualio.sauersack.androidApp.shared.Availability
import cz.visualio.sauersack.androidApp.shared.Container
import cz.visualio.sauersack.androidApp.shared.Content
import cz.visualio.sauersack.androidApp.shared.GeoJsonFeatureProperties
import cz.visualio.sauersack.androidApp.shared.IconType
...
object ThematicParceler : Parceler<Thematic> {
    override fun create(parcel: Parcel): Thematic {
        ...
        val logosUrl = mutableListOf<String>()
        val locationIds = longArrayOf()
        parcel.readStringList(logos)
        parcel.readStringList(logosUrl)
...
enum class Type : Parcelable {
    TEXT, IMAGE, VIDEO, MODEL, FILE, SPHERE_IMAGES
}
...
@Parcelize
@TypeParceler<Content.SphereImages, ContentSphereImagesParceler>
data class ContentSphereImagesWrapper(val value: List<Content.SphereImages>) : Parcelable

object ContentSphereImagesParceler : Parceler<Content.SphereImages> {
    override fun create(parcel: Parcel): Content.SphereImages = Content.SphereImages(
        sphereurl = parcel.readString()!!,
        sort = parcel.readLong(),
        text = parcel.readString()!!,
    )

    override fun Content.SphereImages.write(parcel: Parcel, flags: Int) {
        parcel.writeString(sphereurl)
        parcel.writeLong(sort)
        parcel.writeString(text)
    }
}
...
object ContainerParceler : Parceler<Container> {
    override fun create(parcel: Parcel): Container =
        ...
        when (parcel.readParcelable<Type>(this::class.java.classLoader)!!) {
            ...
            Type.FILE -> parcel.readParcelable<ContentFileWrapper>(this::class.java.classLoader)!!
                .value
                .let(Container::File)
            Type.SPHERE_IMAGES -> parcel.readParcelable<ContentSphereImagesWrapper>(this::class.java.classLoader)!!
                .value
                .let(Container::SphereImages)
        }
    ...
    override fun Container.write(parcel: Parcel, flags: Int) {
        when (this) {
            ...
            is Container.SphereImages -> {
                parcel.writeParcelable(Type.SPHERE_IMAGES, flags)
                ContentSphereImagesWrapper(content)
            }

...
import cz.visualio.sauersack.androidApp.databinding.FragmentLoactionMapBinding
import cz.visualio.sauersack.androidApp.parcelers.LocationParcelable
...
import cz.visualio.sauersack.androidApp.util.LocationClusterRenderer
...
class LocationMapFragment : AnalyticsFragment(Screen.Location.route) {
            ...
            lifecycleScope.launch {
                ...
                vm.flow.mapNotNull {
                    val new = it.toGeoJson()
                        .filter { it.key == thematic.id.toString() }
...
private var currentLayer: GeoJsonLayer? = null
...
private fun updateGeoJson(it: Map<String, GeoJsonFeature>) {
    ...
    features?.let {
        it.onEach { currentLayer?.removeFeature(it) }
    }

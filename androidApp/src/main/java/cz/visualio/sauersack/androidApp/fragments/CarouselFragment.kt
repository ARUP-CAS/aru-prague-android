...
class CarouselFragment : AnalyticsFragment(route = Screen.Gallery.route) {
            ...
            val vpAdapter = ViewPager2Adapter<ContentType>(
                ...
                createFragment = { content ->
                    when (content) {
                        ...
                        is ContentType.SphereImages -> {
                            SphereImagesItemFragment.newInstance(content)
                        }
                    }
                }
            )
            vpAdapter.update(location.toContentType())
            carouselViewPager.offscreenPageLimit = 2
...
fun LocationRes.toContentType(): List<ContentType> = content.flatMap {
    it.content.map {
        when (it) {
            ...
            is Content.SphereImages -> ContentType.SphereImages(
                text = it.text,
                url = it.sphereurl,
                sort = it.sort
